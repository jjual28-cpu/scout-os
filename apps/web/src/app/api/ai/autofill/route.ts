...
import { autofillProduct } from '@/services/ai/autofill';
...
/**
 * POST /api/ai/autofill — suggest category, target, and search keywords for a
 * product in one call. Counts against the caller's daily AI cap. Auth required.
 */
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const result = await autofillProduct(userId, body.product);
  return ok(result);
});
