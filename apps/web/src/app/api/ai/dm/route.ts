...
import { draftDm } from '@/services/ai/dm';
...
/**
 * POST /api/ai/dm — draft a personalized outreach DM for one creator. Counts
 * against the caller's daily AI cap. Auth required; AI failures surface as a
 * normal error the client can show (not a silent skip).
 */
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const text = await draftDm(userId, body.creator, body.brand ?? null);
  return ok({ text });
});
