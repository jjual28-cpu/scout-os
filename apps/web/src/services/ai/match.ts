...
const COMMON_TAIL = `**검색 의도의 '조건'은 주제뿐 아니라 속성도 전부 만족해야 한다 (매우 중요):**
검색 의도에는 콘텐츠 주제(예: 패션) 말고도 아래 같은 '속성 조건'이 함께 들어있을 수 있다. 하나라도
어긋나면 주제가 맞아도 무조건 reject 한다. 주제만 보고 통과시키지 말 것.
- **성별**: 의도에 "여자·여성·여성분·우먼" 또는 "남자·남성·맨"이 있으면, 그 성별의 사람이어야 한다.
  이름·프로필사진·소개글·콘텐츠로 성별이 반대이거나(예: '여자' 검색인데 남성 계정) 불명확하면 reject.
- **국적/지역/언어**: 의도에 "한국·국내·한국인"이 있으면 한국 계정이어야 한다 — 소개글·게시물이 한국어이고
  한국에서 활동하는 계정. 소개글·이름이 영어뿐이거나 명백히 해외(외국어·해외지역) 계정이면 reject.
  (반대로 의도에 특정 국가가 없으면 국적으로 감점하지 말 것.)
- **연령대 등 기타 조건**도 의도에 있으면 동일하게 적용한다.
예) 의도가 "한국 패션 여자 크리에이터"면 (1)한국(한국어·국내) (2)여성 (3)패션 — 셋 다 충족해야만 fit.
남성이거나 외국(영어) 계정이면, 패션 콘텐츠가 아무리 좋아도 reject.
